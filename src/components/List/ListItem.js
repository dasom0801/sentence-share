  ...
  return (
    <li className='list-item'>
      <div className='info'>
        ...
        <Link
          ...
          onClick={(event) => {
            ...
            handleDetailList({
              filter: 'book',
              bookId: item.bookId.replace('/books/', ''),
            });
          }}
        >
          <img
            src={item.bookImage}
            alt={item.bookTitle + '책 표지'}
      ...
      <div className='sentence'>
        ...
        {item.showMoreButton ? (
          <button
            ...
            type='button'
          >
            {/* <FontAwesomeIcon icon={CircleDown} aria-label='문장 더보기' /> */}
            더보기
          </button>
        ) : (
          ''
      ...
      <button
        className='likes'
        type='button'
        onClick={() => handleLikeClick(index, item.id, item.likes, userId)}
      >
        {/* {item.likeUser.indexOf(userId) > -1 ? (
          <FontAwesomeIcon icon={faHeart} aria-label='좋아요 선택' />
        ) : (
          <FontAwesomeIcon
            icon={faHeartRegular}
            aria-label='좋아요 선택하지 않음'
          />
        )} */}
